...
GET_URL()
{
    ...
    echo "$KERNEL_URL" | jq -r --arg i "$1" '.assets[] | select(.name | test($i)) | .browser_download_url' | head -n 1
}
# ]

KERNEL_URL="https://api.github.com/repos/UN1CA/kernel_samsung_s5e8825/releases/latest"
...
unset DTBO_ARCHIVE_URL DTBO_JPN_ARCHIVE_URL KERNEL_ARCHIVE_URL KERNEL_URL
unset -f GET_URL
